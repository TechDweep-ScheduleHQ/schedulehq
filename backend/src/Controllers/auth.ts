...
export const signupByEmail = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (existingUser) {
      // Cache user in Redis
      await Promise.all([
        redisClient.setEx(getUserCacheKey(existingUser.id), CACHE_EXPIRES, JSON.stringify(existingUser)),
        redisClient.setEx(getUserEmailCacheKey(email), CACHE_EXPIRES, JSON.stringify(existingUser)),
        redisClient.setEx(getUsernameCacheKey(username), CACHE_EXPIRES, JSON.stringify(existingUser))
      ])
    ...
    await Promise.all([
      redisClient.setEx(getUserCacheKey(user.id), CACHE_EXPIRES, JSON.stringify(user)),
      redisClient.setEx(getUserEmailCacheKey(user.email), CACHE_EXPIRES, JSON.stringify(user)),
      redisClient.setEx(getUsernameCacheKey(user.username), CACHE_EXPIRES, JSON.stringify(user)),
      redisClient.setEx(getTokenCacheKey(user.id), JWT_EXPIRES_IN, token)
    ])
...
export const verifyEmail = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await Promise.all([
      redisClient.setEx(getUserCacheKey(updatedUser.id), CACHE_EXPIRES, JSON.stringify(updatedUser)),
      redisClient.setEx(getUserEmailCacheKey(updatedUser.email), CACHE_EXPIRES, JSON.stringify(updatedUser)),
      redisClient.setEx(getUsernameCacheKey(updatedUser.username), CACHE_EXPIRES, JSON.stringify(updatedUser)),
      redisClient.setEx(getVerifiedCacheKey(updatedUser.verified), CACHE_EXPIRES, JSON.stringify(updatedUser)),
      redisClient.setEx(getTokenCacheKey(updatedUser.id), JWT_EXPIRES_IN, authToken)
    ])
...
export const googleLoginOAuth = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!user) {
      ...
      if (!dbUser) {
        ...
        await Promise.all([
          ...
          redisClient.setEx(getUserEmailCacheKey(user.email), CACHE_EXPIRES, JSON.stringify(user)),
          redisClient.setEx(getUsernameCacheKey(user.username), CACHE_EXPIRES, JSON.stringify(user)),
          redisClient.setEx(getVerifiedCacheKey(user.verified), CACHE_EXPIRES, JSON.stringify(user))
        ])
    ...
    await Promise.all([
      redisClient.setEx(getTokenCacheKey(user.id), JWT_EXPIRES_IN, authToken),
      redisClient.setEx(getVerifiedCacheKey(user.verified), JWT_EXPIRES_IN, JSON.stringify(user))
    ])
    ...
    return res.status(200).json({
      ...
      user: {
        ...
        availabilities: finalUser?.availabilities || [],
      },
    });
  } catch (error: any) {
    console.error("Google login error:", error.message);
