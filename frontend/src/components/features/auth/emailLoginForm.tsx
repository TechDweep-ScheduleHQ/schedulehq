...
const EmailLoginForm: React.FC = () => {
  ...
  useEffect(() => {
    if (verificationStatus === "success") {
      toast.success(
        verificationMessage || "Email verified successfully! Please log in.",
        {
          position: "top-right",
          autoClose: 3000,
        }
  ...
  useEffect(() => {
    if (isAuthenticated && user) {
      if (user?.availabilities == undefined) return;
      Array.isArray(user?.availabilities) && user?.availabilities?.length > 0
        ? navigate("/dashboard")
        : navigate("/getStarted");
    }
  ...
  if (!googleClientId) {
    console.error(
      "Google Client ID is missing. Please set REACT_APP_GOOGLE_CLIENT_ID in .env file."
    );
    toast.error("Google Sign-In is unavailable due to missing configuration.", {
      position: "top-right",
      autoClose: 3000,
    });
  }
