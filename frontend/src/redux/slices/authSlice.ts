...
export const forgotPassword = createAsyncThunk(
  ...
  async (values: ForgotPasswordValues, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || "Failed to sent Forgot password email!"
      );
    }
  }
);
...
export const fetchTimezones = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.get<TimeZoneResponse>(
        AUTH_URL.TIMEZONES
      );
      ...
      console.log(error.message);
      return rejectWithValue(
        error.response?.data?.message || "Failed to fetch timezone!"
      );
    }
  }
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchTimezones.pending, (state) => {
        (state.timezoneStatus = "pending"), (state.error = null);
      })
